...
2. Checks out the release branch, installs the package (`pip install -e ".[dev]"` so the agent can generate plots) and the **OpenCode CLI** (see `whats-new.yaml` for the exact install step).
3. Runs `python scripts/generate_whats_new.py` with `OPENCODE_MODEL` (default `opencode-go/deepseek-v4-flash`) and a **permissive permission block** (`bash/edit/webfetch/websearch/task` allowed; only `external_directory` denied). The script launches `opencode run` agentically: the agent edits `docs/whats-new.md` directly and decides placement itself, and can generate before/after images under `docs/img/whats-new/`.
4. The agent's stdout is **never** squashed into the file; its edits are the only output. The script only re-asserts the develop/main note banner, validates image references, and fails if `docs/whats-new.md` was not modified. Placement decisions (merge into an existing "Coming in ..." stream section, convert to stable, or add a fresh section) are the agent's, reviewed by the human via the opened PR.

...
Three updates: two fixes to trend metrics and normalised input handling, plus a new user control to toggle noise detection.
...
??? note "Noise detection control (`avoid_noise`)"
    ...
    Useful for treatment signals where you don't care about short-term noise.
    ...
    With the default `avoid_noise=True`, each spike is classified as a Noise segment (20 total fragments).
    Setting `avoid_noise=False` produces 8 clean Up/Down/Flat segments that span right through the spikes.
    ...
    ??? example "Code"
        ...
        # Detect trends, ignoring noise/spike classification

...
Four updates landing in the next stable release: an agentic docs generator, a new noise toggle, and two fixes to trend metrics and normalised input handling.
...
??? note "Trend detection on normalised time series"
    ...
    relative to the signal amplitude — causing the algorithm to detect nothing, which flat fill-in
    then represented as a single all-Flat series.

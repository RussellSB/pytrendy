...
One change in v1.4.0-dev.1: a new `plot_params` dict that gives you full control over plot appearance — figure size, title, axis labels, colour scheme, grid, and legend.

??? note "Plot customisation via `plot_params`"
    A new `plot_params` dictionary, accepted by both `detect_trends()` and `plot_pytrendy()`, lets you override every visual aspect of the trend-detection plot without forking the plotting code. Change the figure dimensions, add a domain-specific title, switch to your organisation's colour palette, toggle grid lines, or reposition the legend — all from the `detect_trends()` call.
    ...
    ??? example "Code"
        ...
        ```
Here is the What's New entry for v1.4.0-dev.1. The images referenced need to be generated by running the code in the `??? example "Code"` block — first with default `plot_params` (before), then with the custom dict (after) — and saving each figure to the paths shown.

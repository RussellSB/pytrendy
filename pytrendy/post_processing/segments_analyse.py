...
def analyse_segments(df: pd.DataFrame, value_col: str, segments: list[dict]) -> list[dict]:
    ...
    - Absolute, percent and multiplier change (based on start/end values)
    ...
    Returns:
        list: 
            ...
            - `'change'`, `'pct_change'`, `'multip_change'`, `'days'`, `'total_change'`, `'SNR'`, `'change_rank'`
    """
    segments_enhanced = []
    for segment in segments:
        ...
        segment_enhanced['multip_change'] = (float(val_end / val_start) if val_start != 0 else np.nan)

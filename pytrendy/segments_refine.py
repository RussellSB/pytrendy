...
def refine_segments(df:pd.DataFrame, value_col: str, segments: list):
    ...
    for i in range(len(segments)):
        ...
        if segment['direction'] == 'Up':
            ...
            temp = df.loc[(pd.to_datetime(segment['start']) - pd.Timedelta(days=7)).strftime('%Y-%m-%d') : (pd.to_datetime(segment['start']) + pd.Timedelta(days=7)).strftime('%Y-%m-%d')].copy()

            lower_start = temp.loc[:, value_col].idxmin() + pd.Timedelta(days=1)
            if lower_start != pd.to_datetime(segment['start']):
                ...
                if prev_exists_and_touching_refined: segments_refined[i-1]['end'] = (lower_start - pd.Timedelta(days=1)).strftime('%Y-%m-%d')

            # End
            temp = df.loc[(pd.to_datetime(segment['end']) - pd.Timedelta(days=7)).strftime('%Y-%m-%d') : (pd.to_datetime(segment['end']) + pd.Timedelta(days=7)).strftime('%Y-%m-%d')].copy()
            
            higher_end = temp.loc[:, value_col].idxmax()
            if higher_end != pd.to_datetime(segment['end']):
                ...
                if next_exists_and_touching_refined: segments_refined[i+1]['start'] = (higher_end + pd.Timedelta(days=1)).strftime('%Y-%m-%d')
        ...
        if segment['direction'] == 'Down':
            ...
            higher_start = temp.loc[:, value_col].idxmax() + pd.Timedelta(days=1)
            if higher_start != pd.to_datetime(segment['start']):
                ...
                prev_distance_refined = (pd.to_datetime(segments_refined[i]['start']) - pd.to_datetime(segment_prev['end'])).days if segment_prev else None
                prev_exists_and_touching_refined = (segment_prev and prev_distance_refined <= 1)
                if prev_exists_and_touching_refined: segments_refined[i-1]['end'] = (higher_start - pd.Timedelta(days=1)).strftime('%Y-%m-%d')

                # Update previous segment if used to overlap (stretch it out)
                # TODO: need to apply for all contractions
                ...
                if prev_exists_and_touching: segments_refined[i-1]['end'] = (higher_start - pd.Timedelta(days=1)).strftime('%Y-%m-%d')


            # End
            temp = df.loc[(pd.to_datetime(segment['end']) - pd.Timedelta(days=7)).strftime('%Y-%m-%d') : (pd.to_datetime(segment['end']) + pd.Timedelta(days=7)).strftime('%Y-%m-%d')].copy()
            
            lower_end = temp.loc[:, value_col].idxmin()
            if lower_end != pd.to_datetime(segment['end']):
                ...
                next_distance_refined = (pd.to_datetime(segment_next['start']) - pd.to_datetime(segments_refined[i]['end'])).days if segment_next else None
                next_exists_and_touching_refined = (segment_next and next_distance_refined <= 1)
                if next_exists_and_touching_refined: segments_refined[i+1]['start'] = (lower_end + pd.Timedelta(days=1)).strftime('%Y-%m-%d')

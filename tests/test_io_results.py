...
class TestPytrendyResults:
    ...
    def test_initialization_with_outlier(self):
        ...
        # Check summary for noise segments
        if 'direction_counts' in results.summary:
            direction_counts = results.summary['direction_counts']
            # Should have detected some noise or abrupt changes
            assert any(d in direction_counts for d in ['Noise', 'Up', 'Down'])
    ...
    def test_set_best_with_trends(self):
        ...
        # Best should be an Up or Down trend (not Flat or Noise)
        assert results.best['direction'] in ['Up', 'Down']
    ...
    def test_set_best_no_trends(self):
        ...
        # With no significant trends, best might be None
        # or might be a flat segment without change_rank
    ...
    def test_set_summary_structure(self):
        ...
        for direction, count in direction_counts.items():
            assert direction in ['Up', 'Down', 'Flat', 'Noise']
    ...
    def test_set_summary_with_outlier(self):
        ...
        # Filter segments to check for noise
        noise_segments = results.filter_segments(direction='Noise', format='dict')
        # The outlier should create some detectable pattern
    ...
    def test_filter_segments_by_direction_noise(self):
        ...
        # Filter for Noise segments
        noise_segments = results.filter_segments(direction='Noise', format='dict')
        
        assert isinstance(noise_segments, list)
        # If noise exists, check structure
        for segment in noise_segments:
            assert segment['direction'] == 'Noise'
    ...
    def test_filter_segments_format_dict(self):
        ...
        segments = results.filter_segments(format='dict')
    ...
    def test_filter_segments_format_df(self):
        ...
        segments_df = results.filter_segments(format='df')
    ...
    def test_segments_have_required_fields(self):
        ...
        required_fields = ['direction', 'start', 'end', 'time_index']

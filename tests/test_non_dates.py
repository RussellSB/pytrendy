...
class TestNonDateCases:
    ...
    @pytest.mark.core
    def test_abrupt_trends_no_padding_integer_index(self):
        """Test that abrupt detection/shaving/cleanup arithmetic works with an implicit
        integer index, matching the equivalent date-indexed result (see
        TestCoreCases.test_abrupt_trends_no_padding) but expressed as day-offsets
        from 2025-01-01 (the first row of the synthetic dataset).
        """
        df = pt.load_data('series_synthetic')
        results = pt.detect_trends(
            df,
        ...
        # Day-offsets from 2025-01-01, mirroring TestCoreCases.test_abrupt_trends_no_padding
        expected_segments = [
            {'direction': 'Flat', 'start': 0, 'end': 57},
            {'direction': 'Up', 'start': 58, 'end': 59},
            {'direction': 'Flat', 'start': 60, 'end': 120},
            {'direction': 'Down', 'start': 121, 'end': 124},
            {'direction': 'Flat', 'start': 125, 'end': 180},
        ]

        assert_segments_match(results.segments, expected_segments)

    @pytest.mark.core
    def test_abrupt_trends_with_padding_integer_index(self):
        """Test that abrupt padding (shave_abrupt_trends + clean_artifacts + update_neighbours)
        works correctly using integer index arithmetic, matching the equivalent date-indexed
        result (see TestCoreCases.test_abrupt_trends_with_padding).
        """
        df = pt.load_data('series_synthetic')
        results = pt.detect_trends(
            df,
        ...
        expected_segments = [
            {'direction': 'Flat', 'start': 0, 'end': 57},
            {'direction': 'Up', 'start': 58, 'end': 87},
            {'direction': 'Flat', 'start': 88, 'end': 120},
            {'direction': 'Down', 'start': 121, 'end': 152},
            {'direction': 'Flat', 'start': 153, 'end': 180},
        ]
